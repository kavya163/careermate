  ...
  <tbody>
   ...
   <?php 
       require 'c.php'; 
            ...
            $sql = "SELECT * FROM report where `cgpa`>='$cgpa'AND `cgpa`<('$cgpa'+1)";
            $result = $con->query($sql);
        while( $row = mysqli_fetch_assoc( $result ) ){
          echo "<tr style='color: white;'><center><td>{$row['rollno']}</td><td>{$row['name']}</td><td>{$row['gender']}</td><td>{$row[ 'dept']}</td><td>{$row['dob']}</td><td>{$row['address']}</td><td>{$row['nationality']}</td><td>{$row['fname']}</td><td>{$row['mname']}</td><td>{$row['contactno']}</td><td>{$row['email']}</td><td>{$row['sem1']}</td><td>{$row['sem2']}</td><td>{$row['sem3']}</td><td>{$row['sem4']}</td><td>{$row['sem5']}</td><td>{$row['sem6']}</td><td>{$row['sem7']}</td><td>{$row['sem8']}</td><td>{$row['cgpa']}</td></center></tr>";  
        }
?>
</tbody>
  </tbody>
